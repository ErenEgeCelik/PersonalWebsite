...
summary: "Over ~84 hours of recorded order-book data we reverse-engineer the dominant market maker as a Brownian-probit pricer, then test every candidate edge. None survives cost-inclusive out-of-sample verification. The negative result is the finding; the protocol that produced it is what transfers."
readingTime: "~15 min"
---

## Abstract

We study the 5-minute up/down binary markets on Bitcoin and Ethereum operated on Polymarket, resolved against the Chainlink BTC/USD/ETH/USD price feeds. Over roughly 84 hours of recorded order-book and multi-exchange tick data (~978 BTC and ~33 ETH healthy slots), we (i) reverse-engineer the dominant market maker's (MM) pricing model, (ii) characterise the latency structure between external price feeds and the on-chain order book, and (iii) conduct an exhaustive, verifier-first search for a tradeable edge. We find that the MM prices each slot as a Brownian-probit digital option, `fair = Φ((F − F₀)/(σ√τ))`, with feed `F = mean(Coinbase, Binance)`, and is **well calibrated** to the Chainlink resolution (Brier 0.2011 vs. our best model's 0.2089 over 404 slots). Every candidate edge we tested failed under out-of-sample, placebo and cost-inclusive verification: directional hold-to-resolution, taker latency arbitrage, spike-scalping, split-sell with profit-lock, passive market making, slot-start cheap-side, dependent-market arbitrage, and six literature-derived hypotheses. The one inefficiency that survives, a feed-to-book reaction lag exceeding 400 ms on 43% of large spikes, is **not capturable** as a taker because of a ~250 ms server-side execution delay, and is weak as a maker because of adverse selection. We conclude the market is efficient at the retail-accessible level. The second half of the paper is a taxonomy of the statistical artifacts that repeatedly manufactured false positives — look-ahead, spread illusion, trend skew, single-session noise, selection bias — and the verifier protocol that caught them.
...
This paper documents a multi-week effort to find and monetise a durable edge in this venue, and the (negative) conclusion that none exists at the retail-accessible level. At least five distinct classes of statistical artifact independently produced "edges" of +1.5c to +9c that evaporated under verification. The negative result is the finding; the order of work that produced it is what transfers.

### 1.1 Central results

1. **The MM is a calibrated Brownian-probit pricer.** Its mid is better calibrated to resolution than any model we built (§4).
2. **A real feed→book reaction lag exists** (43% of large spikes lag >400 ms; §5) but is **uncapturable** because of a fixed ~250 ms server-side taker delay; the lag is mechanical: the book follows the feed rather than anticipating it.
...
4. **Efficiency is uniform** across BTC and ETH and across the slot lifecycle. The only deviations from the model are transient staleness episodes that the latency structure makes unexploitable.
...
- **Execution delay.** A constant ~250 ms server-side taker delay applies to all crypto up/down markets across assets and durations (BTC/ETH/SOL/XRP/BNB/DOGE, 5m and 15m). It is network-independent, confirmed by regional-VPS latency probes, so it is a design property rather than a routing artifact.
...
(36k points) gives median **σ_imp = 5.21** \$/√s against our deployed **σ = 6.06**, a ratio of 0.82. Our σ is too large, which pushes our fair toward 0.50. The MM's implied σ tracks realized variance RV (4.28) rather than jump-robust bipower BV (2.93), at `σ_imp ≈ 1.1·RV`. So it prices in jumps plus about a 20% premium, and our range proxy over-estimates against it. Matching σ to ~1.1·RV aligns our fair to the MM and yields no edge (§6). It reproduces an already-calibrated quote.

### 4.3 Calibration to resolution

Over 404 slots, the **MM mid is better calibrated than our best fair**: Brier 0.2011 (MM) vs 0.2089 (ours); the gap is stable, not noise. The same holds for ETH (MM mid Brier 0.166). The MM's *ask* is calibrated to the realized win-rate (§6.4). There is no static mispricing to fade.
...
A live test (75 fires) found our fill lands ~350 ms after detection, dominated by the ~250 ms server-side taker delay, against a median book reprice ~213 ms after the trigger. We are ~137 ms too slow at the median. The clean capture window covers only ~13% of fires, and the +2c price cap misses 52% of them, which are the winners.

A capture simulation confirmed the trap. With a *forward-looking* spike trigger (knowing the move would reach 4c), taker capture shows +1.5c/share at 350 ms latency. But the trigger is look-ahead. With a causal trigger (fire when the past 1 s moved ≥ threshold), net is **−0.6c to −1.4c**, CI below zero. In real time we cannot select the slow-MM spikes: by the time a move is large enough to identify, the lag is consumed or the move has reverted. (§6/§7.)
...
A proxy metric — in-sample PnL, residual-vs-mid, forward correlation — is not the verifier. The verifier is out-of-sample, cost-inclusive, placebo-controlled net PnL with a slot-clustered confidence interval. Repeatedly the proxy was positive and the verifier was zero or negative.
...
3. **Trend skew.** A directional drift over the sample makes one side's favourites win more; a pooled statistic reads as an edge. Caught by side-split symmetry, where UP-favourite and DN-favourite must agree. This killed the ETH buy-cheap edge (§6.8).
...
- **Cost wall:** the bid–ask spread plus the `p(1−p)·0.07` taker fee is the apparent magnitude of every "edge". The apparent edges *are* the spread (§6.4).

The only positive quantity we measured is the incumbent MM's thin liquidity-provision margin, ~+0.485c/share on enormous volume with high variance. That is a market-making business we cannot contest from a retail position: worse calibration, no queue priority, no rewards pool.
...
- **The only lever that ever existed was latency.** It needs both a fast continuous external feed and an MM that lags it. Both are present in crypto, and the server-side taker delay neutralises them. Event, sports and geopolitical markets have no such feed, so their edge would have to be modelling, which the liquid order books (1c spreads) already price efficiently.
- **Directions requiring an operator decision (not pursued autonomously):**
  1. **Different venue or horizon.** Kalshi, or longer-horizon and lower-liquidity markets where modelling edges may survive. Requires API/auth integration.
  2. **Paid market-making.** Daily commodity and FX up/down markets *are* in the rewards pool, so the edge is the rebate rather than direction. Requires MM infrastructure and inventory risk management.
...
- **What survives the negative result:** a validated MM model card, a latency map, and the verifier protocol and artifact taxonomy of §7, which apply to any short-horizon prediction-market edge search.
...
*A negative-result paper. What it offers is the rigour of the rejection and a reusable protocol, not a trading strategy.*
