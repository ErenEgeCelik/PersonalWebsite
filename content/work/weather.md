...
My working method is to try to find every variable that enters a market's pricing equation, the
whole set rather than a convenient subset of it. Most markets make that impossible; there are too
...
can plausibly enumerate the variable set and then check their model against reality. What follows is
the account of doing that: two edges I traded and measured to death, and a third I built the model
for but never deployed.

Over the period covered here the account grew from roughly $30 to roughly $1,200 across everything I
was running. The absolute numbers are small and the depth available in these books is measured in
hundreds of dollars; what I think the work demonstrates is the process rather than the return.
...
through public channels. No subjective judgment enters anywhere. What is left is a race about a
number that is going to become public regardless.
...
the NOAA path carried it at roughly **five minutes**. That is an advantage of about **four minutes
and forty-four seconds** on the number that settles the contract.
...
**How it died.** Within weeks, market makers found the same feed. What killed the edge was the
structure of the venue rather than their reaction speed. A taker order carries roughly 250–330 ms of
server-side delay before it can match, while a resting maker who sees the same observation can cancel
in 23–50 ms. The asymmetry is about an order of magnitude, and it always favours the side already in
the book. Our orders started returning
`unmatched`: not rejected, not filled, just arrived at a price that no longer existed. A
...
This happened well before the summer, so the pure latency edge had a short life. And the read is not
that we were too slow. We were fast. The venue's taker delay hands the last look to the resting side,
which is a structural feature of the market rather than a latency problem to engineer away.
Recognising that moved the project onto its second phase.
...
| How much size is resting in the book? | A large nominal edge on a contract with $0–40 of depth is not an edge; it is a quote. |
| If we buy, when and at what price can we exit? | Requires a model of how fast the book reprices after an event, which is not a constant. |
...
The decision here is expressed as expected value over an explicit distribution, with execution cost,
liquidity and exit path inside the calculation rather than bolted on afterward. That is the part I
would call trading work rather than engineering.
...
blocking order-status poll took fire latency from **10,163 ms to 58 ms**, a 175× improvement and the
largest execution gain in the project.
...
`Σ_m Q(m)·D_after(k|m) ≈ P_now(k)`, which served as a built-in correctness check. Residual came in
under 2%, the remainder Monte Carlo noise.

**Peak timing.** Because the maximum is monotone, the context that matters most is how much daylight
remains and where the forecast peak sits. A separate component derives the peak hour
...
### 6.1 Falsifying a promising signal
...
best pulled a different winning set on each run, which is the signature of overfitting on correlated
...
constants. No market's dynamics were being fitted at all. I rebuilt the fitter to replay against
archived historical *forecasts*, deliberately not reanalysis, since using realised weather would be
look-ahead leakage. Candidates were scored by Brier.

Mean Brier reduction against the prior model configuration was **24.2%** across 28 fitted markets,
ranging 3.5% to 49%, with no market regressing. The fitter's own behaviour was a useful check: for
one city whose forecast source carries a 5.28°C bias, it independently chose to ignore the forecast
entirely and still improved 35% over the default.

Two limits on that number. It is an offline replay result measured against my own earlier
configuration, not against the market's prices, and I never deployed the final calibration — live
journaling had already stopped by the time the fit was produced.
...
- **Unit mismatch.** Six US markets resolve in Fahrenheit. The forecast drove the state in Fahrenheit
  ...
  never used again. Those six ran on an incoherent posterior for the whole period. I wrote the fix
  and did not deploy it — it touched the money path, and I was holding that class of change for a
  sign-off that never came before the strategy was retired.
...
The scan reconstructs every genuine elimination, meaning a bucket that was alive and leading whose
...
loss costs −1.00. Maker pre-emptive repricing explained only 9% of cases, so this was efficiency
rather than predatory quote-pulling.

*Limitations, stated plainly:* about 22 of the 32 events are independent, since ten are repeated
detections of one persistent situation. The measurement assumes holding to resolution and does not credit
...
- **Circuit breakers.** Per-market consecutive-failure limits, a daily spend cap and a maximum fire
  count, all written after a session in which a single-market bot re-fired every four minutes and
  lost about $80. The failure was not the thesis; it was the absence of a stop.
- **A pre-trade filter.** A pure-orderbook gate requiring clean structure before firing: everything
  below the leader dead, no live contract above the target, the target cheap. It refuses when
...
Two of the three edges I found here are gone and I can say why each one died. The first went to a
structural feature of the venue that hands last look to the resting side. The second went to
efficiency, measured rather than inferred.
...
out-of-sample, paper trade, then size, and I had done the first step and part of the second.

**What I know and what I don't.** Working through this market end to end left me able to think about
these instruments from the maker's side rather than only the taker's: the resolution mechanics, the
observation feeds and their relative latencies, the shape of the books, how quotes behave around a
forced repricing, where informed flow shows up.
...
a proxy source, which I would not repeat.
