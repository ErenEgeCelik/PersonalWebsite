...
short: "A Monte Carlo standings model that reproduced live prices to ±0.02 before I sized it, then traded the lag when goals repriced the match market first. +$16 across four matches on a $30 bankroll."
summary: "Match markets and group-advancement markets price the same underlying uncertainty. When they disagree, the mispricing is mechanical, and a Bayesian link plus a Monte Carlo of the standings makes it explicit."
tags: ["Relative value", "Monte Carlo", "Bayesian"]
---

## Summary

When the reactive edge in the weather markets closed, I went looking for the same structure
elsewhere and found it in the World Cup group stage. This ran with real money in June 2026. It
shares no code with the weather project.

It is not latency arbitrage. It translates information from one market into fair value in another.
...
markets (group winner, advance to knockout, group last place) are thin and slow, and take minutes
to absorb the same information. A Monte Carlo group simulator converts the match market's updated
...
amount, and so has the probability that Paraguay finishes last.
...
against the true ratio of 8/12 = 0.67. The model was wrong in a way that explained itself. I added the third-place term.
...
simulation, which is effectively instant. I verified equivalence against the full simulation: maximum
...
strategy working as designed. The match market said Türkiye was weaker, the model translated
that into the group table, and I took the dependent contract early. But it is also a bet whose
...
contract that rises is DRAW, which was not a trigger. Meanwhile the dependent markets moved
...
Five minutes later France scored, and the position was wrong.

Nothing had happened at minute 84. In a scoreless game the draw becomes more likely because
...
Instead it was harvesting my own disagreement with a market that had no reason to move.
...
eliminating `advance:Ghana` (0.031) and `advance:Panama` (0.030). The computation is free, since the
conditionals already exist.

A difference between your model and the market is not an edge unless you can name the event that
will close it.
...
spread as the only realised effect, which is most of the −4% result.
...
showed the limit of the naive fix. One of the "opposing" buys was a correction, because the
...
I marked the five Australia–Türkiye positions out at the prevailing bid at different holding times:
...
cheap entry also delays the exit. In the weather markets, market makers closed the spread within
seconds.

So this is not an intraday strategy. Positions are held to resolution, with two
...
the profit if the position is ahead and cutting the loss if it is not. The module proved itself in live
...
- **Match selection matters more than execution.** Heavy-favourite matches carry almost no edge:
  the dependent markets have already priced the likely outcome. Balanced matches, where a goal materially
...
- **Kalshi has the markets but not the liquidity.** I surveyed 84 World Cup series there, covering
  match moneylines, group winners, qualification, stage of elimination and group ordering, a more
  complete set than Polymarket's. Every single one had zero volume and no quotes. The fair-value engine is
...
win/draw/loss (winner, advance, last place). Anything requiring a goal-scoring model, such as
highest-scoring group or goal-difference tie-breaks, was left out of scope rather than approximated.

**What I would build next.** Pair-locking, to close out the residual match-outcome variance rather
than carrying it to resolution. A fixture database, to generalise beyond the groups I hand-mapped.
And the edge-decay measurement I eventually ran on the weather strategy, this time before scaling.
